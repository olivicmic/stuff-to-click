...
export default function ColorDragSlider({ value, mode, onChange, slide, ...rest }) {
	...
	const processColor = (input) => {
		colorRgb[channel] = input;
		onChange(Chroma(colorRgb).hex());
	};

	return (
		<DragSlider {...rest} value={colorRgb[channel]} max={255} onChange={processColor} slide={{...slide, backgroundColor: soloColor}} mode={rgb[channel]}/>
	);
};