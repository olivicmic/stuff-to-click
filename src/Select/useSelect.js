...
export default function useSelect({ debug, id, name, onChange, set, valid, value }) {
	...
	const setIndices = (i) => { overlays.update(i); setIndex(i); };
	const close = () => { setActive(false); overlays.remove(overlays.current); setID(); setIndex(-1); };
	const submit = y => { onChange({ target: {...set[y], name }}); close(); };
	const keySubmit = y => { overlays.update(y, set[y]); submit(y); };
